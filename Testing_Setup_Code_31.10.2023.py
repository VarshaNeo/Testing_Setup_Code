...
def Alicat_Sensor_graph_plotting(port_var, sensor_type, data):
    if sensor_type == "Alicat_Sensor":
        if port_var in alicat_data_arrays:
            alicat_data_array = alicat_data_arrays[port_var]
        else:
            alicat_data_array = []
            alicat_data_arrays[port_var] = alicat_data_array
        # Append data to the appropriate array
        alicat_data_array.append(data)
        # Limit the number of data points to display
        data_points_to_display = 9
        if len(alicat_data_array) > data_points_to_display:
            alicat_data_array.pop(0)
        # Plot separate lines for each Alicat sensor
        ax1.clear()
        for port, data_array in alicat_data_arrays.items():
            ax1.plot(Time1[-len(data_array):], data_array, label=f'Alicat Sensor Data ({port})')
        ax1.set_xlabel('Time')
        ax1.set_ylabel('Flow rate')
        ax1.set_title('Alicat flow Sensor Graph (SLPM)')
        plt.xticks(rotation=45)
        ax1.legend()
        canvas1.draw()

CO2_high_data_arrays = {}
CO2_high_time_arrays = {}
Co2_ports = []
def Co2_High_Concentration_sensor_graph_plotting(port_var, sensor_type, data):             #CO2 high concentration sensor graph plotting function
    global Co2_ports
    print("Port inside co2 plotting function=", port_var)
    if sensor_type == "CO2_High":
        if port_var in CO2_high_data_arrays:
            CO2_high_data_array = CO2_high_data_arrays[port_var]
            CO2_high_time_array = CO2_high_time_arrays[port_var]
        else:
            CO2_high_data_array = []
            CO2_high_data_arrays[port_var] = CO2_high_data_array
            CO2_high_time_array = []
            CO2_high_time_arrays[port_var] = CO2_high_time_array
        # Append data to the array
        CO2_high_data_array.append(data)
        CO2_high_time_array.append(time.strftime("%H:%M:%S"))
        data_points_to_display = 9
        if len(CO2_high_data_array) > data_points_to_display:
            CO2_high_data_array.pop(0)
            CO2_high_time_array.pop(0)
        ax2.clear()
        print("datatobecheck",CO2_high_data_arrays[port_var],CO2_high_time_arrays[port_var])
        print("arraydata",CO2_high_data_arrays,CO2_high_time_arrays)
        for port, CO2_high_data_array in CO2_high_data_arrays.items():
            #plot_sensor_data(ax2, Time2, CO2_high_data_array, "label", "Time", "CO2 Concentration", "CO2 High Concentration",data,canvas2)
            ax2.plot(CO2_high_time_arrays[port], CO2_high_data_array, label=port)
        #ax2.plot(Time2,  CO2_high_data_array,label=port_var)
        ax2.set_title("CO2 High Concentration")
...
# co2_low_data_list = []
# Time3 = []
# def CO2_low_concentration_sensor_graph_plotting(port, sensor_type, data):
        ...
        if sensor_type != "Select Sensor" and port:
            ...
            if data is not None:
                if data:                    
                    ...
                    set_sensor_port(sensor_type, port)
                    start_activity()    
                    print("co2 low")                  
        else:
...
def update_graph(port_var, sensor_type):  #stop_update_flag need to be true to run this function
    print("update graph") 
    print("portinsideupdategraphfunction=", port_var )    
    print(stop_update_flag)
    if not stop_update_flag:  # Check the flag
        print(port_var)
        if Upgrade_active: 
            update_all_graphs(port_var, sensor_type)
...
def update_all_graphs(port_var, sensor_type):
    if sensor_type == "Alicat_Sensor":
        data = Alicat_Sensor_Data(port_var)
        print("yesitis right")
        Alicat_Sensor_graph_plotting(port_var, sensor_type,data)
    elif sensor_type == "CO2_High":
        data = float(CO2_High_Concentration_Sensor(port_var))        
        print("co2high isplotting")
        Co2_High_Concentration_sensor_graph_plotting(port_var, sensor_type, data)
    elif sensor_type == "CO2_Low":
        data = CO2_Low_Concentration_Sensor1(port_var)
        CO2_low_concentration_sensor_graph_plotting(port_var, sensor_type, data)
        
# Function to set port values for sensor types
sensor_port_mapping = {}

def set_sensor_port(sensor_type, port):
    if sensor_type and port:
        if sensor_type in sensor_port_mapping:
            if isinstance(sensor_port_mapping[sensor_type], list):
                if port not in sensor_port_mapping[sensor_type]:
                    sensor_port_mapping[sensor_type].append(port)
            else:
                if sensor_port_mapping[sensor_type] != port:
                    sensor_port_mapping[sensor_type] = [sensor_port_mapping[sensor_type], port]
        else:
            sensor_port_mapping[sensor_type] = [port]
        print("Dictionary:", sensor_port_mapping)
